...
def upsert_device(device_id, name, model, android_version):
    ...
    with connect() as conn:
        ...
        if old:
            token = old["token"]
            conn.execute("UPDATE devices SET name=?,model=?,android_version=?,last_seen=?,status='online' WHERE id=?", (name, model, android_version, now, device_id))
        else:
            conn.execute("INSERT INTO devices(id,name,model,android_version,status,last_seen,token,created_at) VALUES(?,?,?,?,?,?,?,?)", (device_id,name,model,android_version,"online",now,token,now))
    return token

def set_device_status(device_id, status):
    with connect() as conn:
        conn.execute("UPDATE devices SET status=?,last_seen=? WHERE id=?", (status,time.time(),device_id))

def create_job(device_id, command, payload=""):
    job_id = secrets.token_hex(10)
    with connect() as conn:
        conn.execute("INSERT INTO jobs(id,device_id,command,payload,created_at) VALUES(?,?,?,?,?)", (job_id,device_id,command,payload,time.time()))
    return job_id
